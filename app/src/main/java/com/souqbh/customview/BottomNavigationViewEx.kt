...
class BottomNavigationViewEx : BottomNavigationView {
    ...
    fun enableShiftingMode(enable: Boolean): BottomNavigationViewEx {
        ...
        //        mMenuView.updateMenuView();
        setLabelVisibilityMode(if (enable) 0 else 1)
        return this
    }
     ...
     * @return
     */
    fun getOnNavigationItemSelectedListener(): BottomNavigationView.OnNavigationItemSelectedListener? {
        // private OnNavigationItemSelectedListener mListener;
        return getField<BottomNavigationView.OnNavigationItemSelectedListener>(
     ...
     * not cause a leak.
     */
    inner private class BottomNavigationViewExOnPageChangeListener(bnve: BottomNavigationViewEx) :
     ...
     */
    inner private class MyOnNavigationItemSelectedListener internal constructor(
        viewPager: ViewPager,
        ...
        init {
            ...
            // create items
            val menu = bnve.getMenu()
            val size = menu.size()
            items = SparseIntArray(size)
            for (i in 0 until size) {
                val itemId = menu.getItem(i).getItemId()
                items.put(itemId, i)
            }
        }
